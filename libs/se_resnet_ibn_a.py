...
def se_resnet50_ibn_a(last_stride, num_classes=1000, pretrained=True, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(last_stride, SEBottleneck, [
                   3, 4, 6, 3], num_classes=num_classes, **kwargs)
...
def se_resnet101_ibn_a(last_stride, num_classes=1000, pretrained=True, ** kwargs):
    ...
    """
    model = ResNet(last_stride, SEBottleneck, [
                   3, 4, 23, 3], num_classes=num_classes, **kwargs)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(
...
def se_resnet152_ibn_a(last_stride, num_classes, pretrained=True):
    ...
    """
    model = ResNet(last_stride, SEBottleneck, [
                   3, 8, 36, 3], num_classes=num_classes)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(
